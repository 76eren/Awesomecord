...
export default function MessageEditDialog({
    ...
    const handleRequestClose = (nextOpen: boolean) => {
        if (!nextOpen) {
            ...
            const base = (initialValue ?? "").trim();
            if (trimmed !== base && trimmed.length > 0) {
                setConfirmOpen(true);
                return;
            }
            setLocalOpen(false);
    ...
    return (
        <Dialog.Root open={localOpen} onOpenChange={handleRequestClose}>
            <Dialog.Portal>
                <Dialog.Backdrop
                    className="fixed inset-0 min-h-dvh bg-gray-50 opacity-20 transition-all duration-150 data-[ending-style]:opacity-0 data-[starting-style]:opacity-0 dark:opacity-70 supports-[-webkit-touch-callout:none]:absolute"
                />
                <Dialog.Popup
                    ...
                    <form
                        ...
                        onSubmit={async (e) => {
                            ...
                            if (!newContent || newContent === (initialValue ?? "").trim()) {
                                setLocalOpen(false);
                                onClose();
                                return;
                            }
                        ...
                        <div className="flex justify-end gap-4">
                            ...
                            <button
                                type="submit"
                                disabled={submitting}
                                className="flex h-10 items-center justify-center rounded-md border border-gray-200 bg-gray-50 px-3.5 text-base font-medium text-gray-900 select-none hover:bg-gray-100 focus-visible:outline focus-visible:outline-2 focus-visible:-outline-offset-1 focus-visible:outline-blue-800 active:bg-gray-100 disabled:opacity-60"
                            >
                                Save
                            </button>
                        </div>
