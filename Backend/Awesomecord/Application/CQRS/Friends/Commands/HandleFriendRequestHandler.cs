...
public sealed class HandleFriendRequestHandler(AppDbContext db, IMapper mapper) : IRequestHandler<HandleFriendRequestCommand, FriendRequestDto>
{
public async Task<FriendRequestDto> Handle(HandleFriendRequestCommand request, CancellationToken ct)
{
    var userThatIsAcceptingOrDenying = await db.Users.SingleOrDefaultAsync(u => u.UserHandle == request.UserThatIsAcceptingOrDenying, ct);
    var userThatIsRequesting = await db.Users.SingleOrDefaultAsync(u => u.UserHandle == request.UserThatIsRequesting, ct);
    ...
    ICollection<FriendRequest> allFriendRequestsByAcceptingParty = userThatIsAcceptingOrDenying.ReceivedFriendRequests;
    ...
    if (action == "accept")
    {
        var alreadyAB = await db.Set<Friendship>().AnyAsync(f => f.UserId == userThatIsAcceptingOrDenying.Id   && f.FriendId == userThatIsRequesting.Id, ct);
        var alreadyBA = await db.Set<Friendship>().AnyAsync(f => f.UserId == userThatIsRequesting.Id && f.FriendId == userThatIsAcceptingOrDenying.Id,   ct);
        ...
        var reverseReq = await db.FriendRequests
            .SingleOrDefaultAsync(fr => fr.RequesterId == userThatIsRequesting.Id && fr.RecipientId == userThatIsAcceptingOrDenying.Id, ct);

        if (reverseReq is not null)
        {
            db.Set<FriendRequest>().Remove(reverseReq);
        }
    }
